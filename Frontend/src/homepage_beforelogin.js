...
const FloatingMenu = ({
  ...
  useLayoutEffect(() => {
    ...
    // (A full cleanup function would be better in a real app, but for simplicity we'll proceed)
    
    if (isOpen) {
      cards.forEach((card, i) => {
        // Use a slight delay for staggered entry animation
        setTimeout(() => {
          ...
          {(items || []).slice(0, 5).map((item, idx) => (
            ...
            >
              ...
              <div style={{
                ...
                {/* Sub-links will now remain functional, but they should ideally only be present
                  if you want internal links within the card. If the entire card is a link,
                  you might want to remove the sub-links.
                */}
                {item.links?.map((lnk, i) => (
                  <span // Changed to span to prevent nested interactive elements (a inside a)
                    key={`${lnk.label}-${i}`} 
                    // To handle clicks on these sub-links, you'd add an onClick handler:
                    // onClick={(e) => { e.stopPropagation(); window.location.href = lnk.href; }}
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: '0.5rem',
  ...
  const items = [
    {
      ...
      links: [
        ...
        { label: "use Tool", ariaLabel: "use tool", href: "#company" },
      ...
      <div style={styles.contentOverlay}>
        ...
        <main style={styles.mainContent}>
          <div style={styles.contentWrapper}>
            <h1 style={styles.mainTitle}><img src="mainlogo.png" style={{ height: "100px", width: "auto" }} /></h1>
